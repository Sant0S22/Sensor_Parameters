...
class Db_Cleaner :
    ...
    def create_map_anomaly ( self ) :
        ...
        data_path = str(self.inizio)
        tmp = str(self.o_inizio).split(":")
        data_path = data_path + "_{}_{}_{}".format(tmp[0],tmp[1],tmp[2])
    ...
    def cleanData ( self ) :
        ...
        sql = self.sql_max_min.format(self.puntiTemporanea,self.misureTemporanea,self.puntiTemporanea,self.misureTemporanea,self.inizio,self.fine,self.o_inizio,self.o_fine)
        ...
        for i in range(self.min_x , self.max_x , self.range_x ) :
            for j in range (self.min_y , self.max_y , self.range_y ) :
                #print ( str(i) + " " + str(j))
                tmp = self.sql_query_range.format(self.puntiTemporanea,self.misureTemporanea,self.misureTemporanea,self.puntiTemporanea,self.inizio,self.fine,self.o_inizio,self.o_fine,i,i+self.range_x,j,j+self.range_y)
                #print(sqlRange)
